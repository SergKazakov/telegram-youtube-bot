...
export const onFeed = handleError(
  async ({ topic, feed }: { topic: string; feed: Buffer }) => {
    ...
    if (dayjs().diff(dayjs(published), "day", true) > 1) {
      return
    }
    ...
    await redis.setex(videoId, 30 * 24 * 60 * 60, 1)
